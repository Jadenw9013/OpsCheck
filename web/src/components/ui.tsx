...
export function Pill({
  ...
  return (
    <span
      className={
        'inline-flex items-center gap-2 rounded-full border px-3 py-1 ops-meta font-medium ' +
        TONE_CLASS[tone]
      }
    >
      {glyph}
...
export function Panel({
  ...
  return (
    <section aria-label={title} className={'ops-panel flex min-h-0 flex-col ' + className}>
      <header className="flex items-start justify-between gap-4 px-6 py-5">
        <div className="min-w-0">
          <h2 className="ops-panel-title text-ink">{title}</h2>
          {subtitle ? (
            <p className="ops-body mt-1 max-w-[65ch] text-ink-soft">{subtitle}</p>
          ) : null}
        </div>
